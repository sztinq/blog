...
def global_setting(request):
    ...
    # 浏览排行 前5
    article_view_list = Article.objects.all().order_by('-click_count')[:5]
    # 评论排行
...
def article(request):
    try:
        ...
        for comment in comments:
            try:
                avatar = User.objects.get(username=comment.username).avatar
            ...
            if user_avatar not in user_list:
                user_list.append(user_avatar)
