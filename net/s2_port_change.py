...
if s2_ip:
    ...
    # print("User: " + "IP-" + user_ip + " MAC-" + user_mac)
    print "用户接入交换机信息：【名称: " + sw_name + " IP地址：" + s2_ip + " 端口:" + sw_port + "】"
    command_int = "int g1/0/" + sw_port.split('/')[2]
    tn.write("sys" + enter_key)
    time.sleep(1)
    tn.write(command_int + enter_key)
    time.sleep(1)
    tn.write(s2_interface_command + enter_key)
    #print(sw_port + "执行命令为：" + s2_interface_command)
    time.sleep(1)
    res = tn.read_very_eager()
    print res

    time.sleep(1)
    tn.write("save" + enter_key)
    time.sleep(1)
    res = tn.read_very_eager()
    if res.find("Y/N"):
        tn.write('y' + enter_key)
        time.sleep(1)
        tn.write(enter_key)
        print(sw_name + "保存成功！")
